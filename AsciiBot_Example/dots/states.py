...
class TravelState(State):
    ...
    def next(self, char):
        if char == ' ':
            ...
            self.parent.env.interpreter.yield_control = True
            return DeadState(self.parent)
...
class ValueState(State):
    ...
    def run(self, char):
        if char.isdecimal():
            ...
            try:
                self.parent.value = self.parent.env.interpreter.input[self.parent.id]
            except ValueError:
                self.parent.value = 0
...
class PrintState(State):
    ...
    def run(self, char):
        if char == '_':
            ...
            try:
                self.parent.env.interpreter.output[self.parent.id] = self.parent.value
            except:
                pass
