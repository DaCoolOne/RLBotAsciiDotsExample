...
class AsciiDotsInterpreter(object):
    def __init__(self, env, program, program_dir, run_in_parallel):
        ...
        self.input = []
        self.output = [0, 0, 0, 0, 0, False, False, False, False]
    ...
    def step(self):
        ...
        """
        with self:
            while not self.needs_shutdown and not self.yield_control and len(self.env.dots) > 0 and not all(dot.state.isWaiting for dot in self.env.dots):
                if self.run_in_parallel:
                    self.parallel_tick()
                else:
                    self.async_tick()
